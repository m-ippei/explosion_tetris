function preload() {
  img[0] = loadImage('background.png');
  img[1] = loadImage('red.png');
  img[2] = loadImage('green.png');
  img[3] = loadImage('blue.png');
  img[4] = loadImage('yellow.png');
  img[5] = loadImage('keyboard.png');
}
...
function explositonProcess(){
  t+=0.2
	...
	for(var i=0;i<eI.length;i++){
		...
		Rect(Rs[i][j].x+aX,Rs[i][j].y+aY,Field[eI[i]][j+1])
		pop()
 	  }	
	}
}